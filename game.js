...
function requestWarrant() {
  // Solicita mandado apenas se houver caso em investigação e um alvo selecionado (suspeito ou testemunha)
  if (!currentCase) {
    alert("Nenhum caso em investigação no momento.");
    return;
  }
  if (!currentSuspect && !currentWitness) {
    alert("Selecione um suspeito ou testemunha para solicitar mandado.");
    return;
  }
  // Simular aprovação do juiz
  ...
  // Caso aprovado, realizar busca no alvo selecionado
  const target = currentSuspect || currentWitness;
  const foundEvidence = Math.random() < 0.5;
  if (foundEvidence) {
    alert(`Busca realizada na residência de ${target}. Uma nova prova foi encontrada.`);
    // Como exemplo, aumentar prestígio e reputação
    gameState.prestige = Math.min(100, gameState.prestige + 5);
    gameState.reputation = Math.min(100, gameState.reputation + 4);
    // Poderíamos adicionar uma nova prova à lista, mas sem novos arquivos fica apenas mensagem
  } else {
...
function endExam() {
  ...
  if (percent >= 70) {
    ...
    alert(`Parabéns! Você foi aprovado(a) no exame com ${percent.toFixed(0)}%. Agora você é ${selectedAgency}.`);
  } else {
    alert(`Infelizmente você obteve ${percent.toFixed(0)}% e não atingiu os 70% necessários.`);
  }
  // Resetar seleção
  selectedAgency = "";
  examQuestions = [];
...
function openStats() {
  ...
  if (nextEl) {
    if (gameState.agency === "police") {
      ...
      if (casesNeed === 0 && prestigeNeed === 0) {
        nextEl.textContent = "Você já pode solicitar promoção para FBI/CIA.";
      } else {
        nextEl.textContent = `Falta(m) ${casesNeed} caso(s) resolvido(s) e ${prestigeNeed}% de prestígio para a próxima promoção.`;
      }
...
function openPromotion() {
  // Exibe a tela de promoção se critérios básicos forem atendidos
  const msgEl = document.getElementById("promotionMessage");
  const optionsEl = document.getElementById("agencyOptions");
  let eligible = false;
  if (gameState.agency === "police") {
    // Para primeira promoção (FBI ou CIA), exige 2 casos resolvidos e prestígio >= 70
    eligible = gameState.solved >= 2 && gameState.prestige >= 70;
  } else if (gameState.agency === "FBI") {
    // Para segunda promoção (CIA), exige 4 casos resolvidos e prestígio >= 80
    eligible = gameState.solved >= 4 && gameState.prestige >= 80;
  } else {
    eligible = false;
  }
  if (!msgEl || !optionsEl) {
  ...
  if (gameState.agency === "CIA") {
    ...
    msgEl.textContent = "Escolha a agência para a qual deseja se promover:";
    // exibir opções como flex para alinhar os cartões
    optionsEl.style.display = "flex";
  } else {
    msgEl.textContent = "Você ainda não tem requisitos para promoção. Resolva mais casos e aumente seu prestígio.";
    optionsEl.style.display = "none";
  }
