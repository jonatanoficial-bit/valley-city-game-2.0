...
function openPromotion() {
  ...
  let eligible = false;
  if (gameState.agency === "police") {
    // Para primeira promoção (FBI ou CIA), exige 2 casos resolvidos, prestígio >= 70 e pontuação >= 60
    eligible = gameState.solved >= 2 && gameState.prestige >= 70 && gameState.score >= 60;
  } else if (gameState.agency === "FBI") {
    // Para segunda promoção (CIA), exige 4 casos resolvidos, prestígio >= 80 e pontuação >= 80
  ...
  if (gameState.agency === "CIA") {
    ...
    msgEl.textContent = "Você ainda não tem requisitos para promoção. Resolva mais casos, aumente seu prestígio e pontuação.";
    optionsEl.style.display = "none";
  }
